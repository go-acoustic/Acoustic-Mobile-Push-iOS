...
-(IBAction)enlargeImage:(id)sender
{
    ...
    if(self.fullScreen) {
        NSString * imageUrlString = self.message ? self.message.content[@"contentImage"] : nil;
        viewController = [[MCEInboxPostTemplateImage alloc] initWithNibName:@"MCEInboxPostTemplateImage" bundle:nil imageUrlString: imageUrlString];
    } else {
        id <MCETemplateDisplay> vc = [MCETemplateRegistry.sharedInstance viewControllerForTemplate: self.message.templateName];
        [vc setInboxMessage: self.message];
        viewController = (UIViewController*) vc;
    }
    
    if(!viewController) {
        NSLog(@"Can't generate view controller for inbox message");
        return;
    }
    
    UIViewController * controller = MCESdk.sharedInstance.findCurrentViewController;
    if(!controller) {
        NSLog(@"Can't find current view controller to present inbox message");
        return;
    }
...
-(void)updateTheme {
    self.subHeader.textColor = [UIColor lightThemeColor:UIColor.grayColor darkThemeColor:UIColor.lightGrayColor];
}
