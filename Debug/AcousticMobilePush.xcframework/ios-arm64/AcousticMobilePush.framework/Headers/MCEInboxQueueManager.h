...
typedef void (^MCEMessageCallback)(MCEInboxMessage * _Nullable message, NSError * _Nullable error);
...
@property(class, nonatomic, readonly) MCEInboxQueueManager * _Nonnull sharedInstance NS_SWIFT_NAME(shared);
 ...
 */
- (void) getInboxMessageId: (NSString * _Nonnull) inboxMessageId completion: (MCEMessageCallback _Nonnull ) callback;
