...
class GeofenceVC: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate, UIGestureRecognizerDelegate {
    ...
    func updateLocation(region:  MKCoordinateRegion, location: CLLocation) {
    ...
    @objc func didDragMap(gestureRecognizer: UIGestureRecognizer) {
        if(gestureRecognizer.state == UIGestureRecognizer.State.ended) {
            ...
            if lastLocation == nil {
                updateLocation(region: region, location: location)
            } else if let lastLocation = lastLocation, lastLocation.distance(from: location) > 10 {
                updateLocation(region: region, location: location)
            }
        }
    }
